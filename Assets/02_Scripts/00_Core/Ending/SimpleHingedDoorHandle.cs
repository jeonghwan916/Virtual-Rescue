...
public sealed class SimpleHingedDoorHandle : MonoBehaviour
{
    ...
    [Header("References")]
    [SerializeField] private SimpleHingedDoorController _doorController;
    ...
    private void Reset()
    {
        ...
        _doorController = GetComponentInParent<SimpleHingedDoorController>();
    }
    ...
    private float CalculateAngleDelta(Vector3 previousPosition, Vector3 currentPosition)
    {
        ...
        Vector3 previousOffset = Vector3.ProjectOnPlane(
            previousPosition - _handlePivot.position, worldAxis);
        Vector3 currentOffset = Vector3.ProjectOnPlane(
            currentPosition - _handlePivot.position, worldAxis);
    ...
    private void ReturnToNeutral()
    {
        ...
        SetAngle(Mathf.MoveTowards(
            _currentAngle, 0f, _returnSpeed * Time.deltaTime));
    }
    ...
    private void ApplyRotation()
    {
        Quaternion angleRotation =
            Quaternion.AngleAxis(_currentAngle, GetLocalRotationAxis());
    ...
    private void ValidateReferences()
    {
        ...
        {
            Debug.LogError(
                "SimpleHingedDoorHandle requires a door controller.", this);
        }

        if (_interactable == null)
        {
            Debug.LogError(
                "SimpleHingedDoorHandle requires an XRSimpleInteractable.", this);
        }

        if (_handlePivot == null)
        {
            Debug.LogError(
                "SimpleHingedDoorHandle requires a handle pivot.", this);
        }
    }
    ...
    private void OnValidate()
    {
        ...
        float maximumAbsoluteAngle =
            Mathf.Max(Mathf.Abs(_minimumAngle), Mathf.Abs(_maximumAngle));
